...
/*    @Produces(MediaType.APPLICATION_JSON+";charset=UTF-8")
    ...
    public String calculateRiskValue(@FormParam("reportId") String reportId){
        ...
        for(CraneInspectReport cr:craneInspectReportList){
            ...
            if(uploadReport.get("status").equals("报告未计算")){
            //批量插入riskValue
            if(craneInspectReportService.InsertToRiskValue(mapList)){
                //更新
              craneInspectReportService.updateUploadedReportByReportId(Long.parseLong(str[i]),"报告已计算");
            };
        }else{
            //将重复的记录保存到list中
    ...
    @Produces(MediaType.APPLICATION_JSON+";charset=UTF=8")
    @POST
    @Path("/calculateProvinceCityAreaRisk")
    public String calculateProvinceCityAreaRisk(){
     return null;
    }
}
