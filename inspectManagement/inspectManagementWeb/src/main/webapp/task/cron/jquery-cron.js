...
(function($) {
    ...
    var str_opt_mih = "";
    for (var i = 1; i <= 24; i++) {
        str_opt_mih += "<option value='"+i+"'>" + i + "</option>\n";
    }

    // options for hours in a day
    var str_opt_hid = "";
    for (var i = 1; i <= 24; i++) {
        str_opt_hid += "<option value='"+i+"'>" + i + "</option>\n";
    }

    // options for days of month
    var str_opt_dom = "";
    for (var i = 1; i < 32; i++) {
        if (i == 1 || i == 21 || i == 31) { var suffix = "号"; }
        else if (i == 2 || i == 22) { var suffix = "号"; }
        else if (i == 3 || i == 23) { var suffix = "号"; }
        else { var suffix = "号"; }
    ...
    var methods = {
        init : function(opts) {
            ...
            block["mins"] = $("<span class='cron-block cron-block-mins'>"
                    + " 第 <select name='cron-mins'>" + str_opt_mih
                    + "</select> 分钟 </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["time"] = $("<span class='cron-block cron-block-time'>"
                    + " 从 <select name='cron-time-hour' class='cron-time-hour'>" + str_opt_hid
                    + "</select>点 到<select name='cron-time-min' class='cron-time-min'>" + str_opt_mih
                    + " </select>点</span>")
                .appendTo(this)
                .data("root", this);
        ...
        getCronValue:function(){
            return methods.value.call(root.data("current_value"));
        }
